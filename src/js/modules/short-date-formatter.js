...
/**
 * Convert text like "5 hours ago" to "5h ago"
 */
function convertToShortFormat(text) {
  ...
  for (const pattern of patterns) {
    if (pattern.replacement) {
      // Direct replacement (like "a minute ago" -> "1m ago")
      if (pattern.regex.test(text)) {
        return pattern.replacement + ' ago';
      }
    } else {
      // Extract number and add suffix
      const match = text.match(pattern.regex);
      if (match) {
        return match[1] + pattern.suffix + ' ago';
      }
    }
  }
...
export function formatShortTimeago(dateString) {
  ...
  if (diffInSeconds < minute) {
    shortText = `${diffInSeconds}s ago`;
  } else if (diffInSeconds < hour) {
    shortText = `${Math.floor(diffInSeconds / minute)}m ago`;
  } else if (diffInSeconds < day) {
    shortText = `${Math.floor(diffInSeconds / hour)}h ago`;
  } else if (diffInSeconds < week) {
    shortText = `${Math.floor(diffInSeconds / day)}d ago`;
  } else if (diffInSeconds < month) {
    shortText = `${Math.floor(diffInSeconds / week)}w ago`;
  } else if (diffInSeconds < year) {
    shortText = `${Math.floor(diffInSeconds / month)}mo ago`;
  } else {
    shortText = `${Math.floor(diffInSeconds / year)}y ago`;
  }
    ...
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          const dateElements = node.querySelectorAll(
            'time[datetime], .timeago, [data-timeago]',
          );
          ...
          if (
            node.matches &&
            node.matches('time[datetime], .timeago, [data-timeago]')
          ) {
