...
export class BlogPostDisplay {
  constructor(options = {}) {
    this.options = {
      ...
      contentSelector: '.post-content', // Use the existing content div
  ...
  shouldIgnorePost(post) {
    ...
    for (const tag of this.options.ignoredPrimaryTags) {
      if (
        post.classList.contains(`tag-${tag.toLowerCase()}`) ||
    ...
    if (
      primaryTag &&
      this.options.ignoredPrimaryTags.includes(primaryTag.toLowerCase())
    ) {
      return true;
    }

    // Method 3: Check for tag link in post meta (common Ghost pattern)
    const tagLinks = post.querySelectorAll(
      '.post-tags a, .post-meta a[href*="/tag/"]',
    );
      ...
      if (href) {
        for (const tag of this.options.ignoredPrimaryTags) {
          if (
            ...
            const allTagLinks = post.querySelectorAll(
              '.post-tags a, .post-meta a[href*="/tag/"]',
            );
  ...
  processNewlyLoadedPosts() {
    ...
    const unprocessedPosts = document.querySelectorAll(
      `${this.options.postSelector}:not([data-word-count])`,
    );
    ...
    console.log(
      `BlogPostDisplay: Processing ${unprocessedPosts.length} newly loaded posts`,
    );
    ...
    unprocessedPosts.forEach((post, index) => {
      try {
        ...
        console.error(
          `BlogPostDisplay: Error processing new post ${index}:`,
          error,
        );
      }
    });
  ...
  processAllPosts() {
    ...
    if (posts.length === 0) {
      console.warn(
        'BlogPostDisplay: No posts found with selector:',
        this.options.postSelector,
      );
      return;
    }
    ...
    posts.forEach((post, index) => {
      try {
        ...
        console.error(
          `BlogPostDisplay: Error processing post ${index}:`,
          error,
        );
      }
    });
    ...
    console.log(
      `BlogPostDisplay: Processed ${posts.length} posts on blog index`,
    );
  }
  ...
  processPost(post, index) {
    ...
    if (this.shouldIgnorePost(post)) {
      console.log(
        `Post ${index}: Ignoring post with primary tag in ignore list`,
      );
    ...
    const contentElement =
      post.querySelector(this.options.contentSelector) ||
    ...
    if (!contentElement) {
      console.warn(
        `BlogPostDisplay: No content element found in post ${index}. Tried selectors:`,
    ...
    if (!previewElement || !fullElement) {
      console.warn(
        `BlogPostDisplay: Missing preview or full element in post ${index}:`,
        {
  ...
  processSpecificPosts(posts) {
    ...
    posts.forEach((post, index) => {
      try {
        ...
        console.error(
          `BlogPostDisplay: Error processing manual post ${index}:`,
          error,
        );
      }
    });
  ...
  updateIgnoredTags(newIgnoredTags) {
    this.options.ignoredPrimaryTags = newIgnoredTags.map((tag) =>
      tag.toLowerCase(),
    );
  ...
  getStats() {
    const allPosts = document.querySelectorAll(
      `${this.options.postSelector}[data-word-count]`,
    );
    const ignoredPosts = Array.from(allPosts).filter(
      (post) => post.getAttribute('data-word-count') === 'ignored',
    );
    const processedPosts = Array.from(allPosts).filter(
      (post) => post.getAttribute('data-word-count') !== 'ignored',
    );
